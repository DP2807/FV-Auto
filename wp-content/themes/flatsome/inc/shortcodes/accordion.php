...
function ux_accordion_item( $atts, $content = null ) {
	...
	return '<div id="' . esc_attr( $id ) . '" class="' . implode( ' ', $classes ) . '"><a id="' . esc_attr( $id ) . '-label" href="#" class="' . implode( ' ', $title_classes ) . '" aria-expanded="' . ( $is_open ? 'true' : 'false' ) . '" aria-controls="' . esc_attr( $id ) . '-content"><button class="toggle" aria-label="' . esc_attr__( 'Toggle', 'flatsome' ) . '"><i class="icon-angle-down"></i></button><h3>' . $title . '</h3></a><div id="' . esc_attr( $id ) . '-content" class="accordion-inner"' . ( $is_open ? ' style="display: block;"' : '' ) . ' aria-labelledby="' . esc_attr( $id ) . '-label">' . do_shortcode( $content ) . '</div></div>';
}
